...
# Copy of process_data_2 with input values for 720 minute intervals, change 3600 to 43200, change default 24 to 14 (1 week)



def fill_parquet(input_path: str, folder: str = None, timestep: int = 43200) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        timestep: Time step in seconds between consecutive timestamps 
    """
    # Read input file
...
def compute_ma_zscore(close: pd.Series, window: int = 14) -> Tuple[pd.Series, pd.Series]:
...
def price_ema_diff(price: pd.Series, window: int = 14) -> pd.Series:
    """
...
def compute_volatility(returns: pd.Series, volume: pd.Series, window: int = 14) -> Tuple[pd.Series, pd.Series]:
...
def compute_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    config = {
        'momentum': [(2, 1), (14, 2), (12, 7), (36, 13)],  # (a, b) pairs for momentum
        'reversal': [6, 28],    # lookback window for ST_Rev and LT_Rev
        'ema_diff': [(10, 21), (20, 50)],  # (fast, slow) pairs for ema_diff
        #'price_ema': [14],                  # windows for price_ema_diff
        #'stoch_osc': [12],              # windows for stochastic oscillator
        #'amihud': [12],                     # windows for amihud illiquidity
        #'volatility': [14],                 # windows for realized and weighted volatility
        #'ma_zscore': [14],                  # windows for MA and z-score
        #'rsi': [14],                        # windows for RSI
        #'macd': [(12, 26)]              # (fast, slow, signal) tuples for MACD
    }
    
    window = 14 # default window size
    
    # Compute volatility metrics first
    realized_vol, weighted_vol = compute_volatility(returns, volume, window)
    ...
    indicators_dict = {
        ...
        # MA z-score
        f'ma_zscore_{window}': compute_ma_zscore(close, window),
        ...
        **{f'mom_{a}_{b}': compute_momentum(close, a, b) 
        ...
        f'ltrev_{config["reversal"][1]}': -compute_momentum(close, config['reversal'][1], 1), # [t-a, t-b] with b=1, a in config['reversal']
        
        # EMA-based indicators
        **{f'ema_diff_norm_{fast}_{slow}': ema_diff(close, fast, slow, 'price') for fast, slow in config['ema_diff']},
        f'price_ema_diff_{window}': price_ema_diff(close, window),
        f'macd_hist': macd_hist(close),
        
        # Volatility and volume-based indicators
        f'realized_vol_{window}': realized_vol,
        f'weighted_vol_{window}': weighted_vol,
        f'volume_zscore_{window}': volume_zscore(volume, window),
        f'vwap_{window}': compute_vwap(high, low, close, volume, window),
        
        # Stochastic oscillator
        f'sto_osc_{window}': stochastic_oscillator(high, low, close, window),
        
        # RSI
        f'rsi': compute_rsi(close),
        
        # Amihud illiquidity
        f'amihud_{window}': amihud_illiquidity(log_returns, volume, window)
    }
...
def process_single_file(filepath: str, output_folder: str = None) -> pd.DataFrame:
    ...
    if output_folder is not None:
        ...
        # Overwrite the current file if no output folder specified
        df.to_parquet(filepath, index=False)
    
    return df


def process_folder(input_folder: str, output_folder: str = None, max_workers: int = None) -> None:
    """
    Process all parquet files in the input folder in parallel and save results to output folder.
    """
    # Get all parquet files in input folder
    filepaths = [os.path.join(input_folder, f) for f in os.listdir(input_folder) if f.endswith('.parquet')]
